<script lang="ts">
	...
	const canonicalUrl = absoluteUrl(`/photowall/${data.post.slug}`);
	const title = buildPageTitle(data.post.title);
	const description = data.post.description ?? `${data.post.title} photo post by ${site.personName}.`;
	const firstImage = data.post.coverImage ? absoluteUrl(data.post.coverImage) : undefined;
	const structuredData = serializeJsonLd([
		{
			...
			image: data.post.images.map((image) => absoluteUrl(image.src))
		},
		breadcrumbJsonLd([
			{ name: 'Home', path: '/' },
			{ name: 'Photowall', path: '/photowall' },
			{ name: data.post.title, path: `/photowall/${data.post.slug}` }
		])
	]);
</script>
